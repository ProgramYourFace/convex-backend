...
`/health/storage` writes and fsyncs a small file on the blocking pool — a read of a file
the process has already touched is answered from cache, and proves nothing — and is the
