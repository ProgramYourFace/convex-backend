    ...
    pub(crate) fn spawn(
        ...
        let handle = std::thread::Builder::new()
            ...
            .spawn(move || loop {
                ...
                check(&inner, &shutdown, backup_dir.as_deref(), started, &instance);
            })
...
fn check(
    ...
    metrics::log_oldest_write(instance, stalled.map_or(0.0, |d| d.as_secs_f64()));
    ...
    metrics::log_write_stopped(instance, engine_is_applying_backpressure(inner));
    if errors > 0 {
        ...
        && let Some(worker) = inner.backup_worker.get()
    {
        ...
        metrics::log_backup_age(instance, age);
    }
}
